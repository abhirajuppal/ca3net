...
def learning(spiking_neurons, spike_times, taup, taum, Ap, Am, wmax, w_init, run_time):
    """
...
if __name__ == "__main__":
    ...
    # Build the network explicitly
    device.build()

    for shots in [1, 2, 4, 8, 12, 32, 50]:
        run_time = 9.2 * shots
        place_cell_ratio = 0.5
        linear = True
        f_in = "spike_trains_%.1f_linear.npz" % place_cell_ratio if linear else "spike_trains_%.1f.npz" % place_cell_ratio
        f_out = "wmx_%s_%.1f_%dshots_linear.npz" % (STDP_mode, place_cell_ratio, shots) if linear else "wmx_%s_%.1f.pkl" % (STDP_mode, place_cell_ratio)
        ...
        weightmx = learning(spiking_neurons, spike_times, taup, taum, Ap, Am, wmax, w_init, run_time)
